...
class Ui_Form(QWidget):
    def setupUi(self, Form):
        ...
        self.TitleLabel.setGeometry(QtCore.QRect(370, 60, 200, 150))
        font = QtGui.QFont()
    ...
    def get_candidate_keys(self,r,fd):  
        dic = self.closure(r)
        temp_fd_lst = []
    ...
    def get_prime_attributes(self,r,fd):
        retset = set([])
        
        keys = self.get_candidate_keys(r,fd)

        for key in keys:
            for a in key:
                retset.add(a)
        retset = sorted(retset)
        return retset

    def get_nonprime_attributes(self,r,fd):
        primes = self.get_prime_attributes(r,fd)
        return set(r) - set(primes)

    def is_2NF(self,r,fd):
        candidates = self.get_candidate_keys(r,fd)
        primes = self.get_prime_attributes(r,fd)
        non_primes = self.get_nonprime_attributes(r,fd)
        ...
        for i in fd:
            ...
            for att in non_primes:
                if att in rhs:
                    s = "\n" + str(lhs) + "->" + str(rhs) + " breaks 2NF requirements (" + str(att) + " is non-prime )"
                    return s

        return "The relation is in 2NF"

    def is_3NF(self,r,fd):
        superkeys = self.get_super_keys(r,fd)
        primes = self.get_prime_attributes(r,fd)
        non_primes = self.get_nonprime_attributes(r,fd)
        ...
        return "The relation is in 3NF"

    def is_BCNF(self,r,fd):
        superkeys = self.get_super_keys(r,fd)
        for i in fd:
            lhs = i
            rhs = fd[i]
            ...
            if lhs not in superkeys:
                s = "\n" + str(lhs) +  "->" + str(rhs) + " breaks 2NF requirements (" + str(lhs) + " is non-superkey )"
                return s

        return "The relation is in BCNF"
            ...
            if question == QMessageBox.Yes:
                ...
                for i in st:
                    if i not in r:
                        ...
                        mess.setText("Attribute " + i + " not in R")
                        mess.exec_()
                        return
                ...
                for i in range(len(key)):
                    x = "K"+str((i+1))+": "+key[i]+'\n'
                    display_key.append(x)
                display_r = r
                display_r = 'R(' + display_r +')\n'
                display_fd = ''
                for i in fd:
                    y = i + ' -> ' + fd[i] + '\n'
                    display_fd += y
                ...
                for i in display_key:
                    display_str_key += i
                is2NF = self.is_2NF(r,fd)
                is3NF = self.is_3NF(r,fd)
                isBCNF = self.is_BCNF(r,fd)
                display_all = display_r + display_fd + display_str_key + is2NF + "\n" + is3NF + "\n" + isBCNF
                self.KeyLine.setText(display_all)
    ...
    def retranslateUi(self, Form):
        ...
        self.TitleLabel.setText(_translate("Form", "Key + NF"))
